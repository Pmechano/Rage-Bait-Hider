# Rage Bait Hider（油猴试用版 v0.3.2）

在 B 站普通视频页自动过滤评论、楼中楼和普通弹幕。使用 Jev 的一个 `should_hide` 含义的是非判断，默认概率达到 **0.30** 就隐藏。未完成判断、判断缺失、网络失败的内容也保持隐藏。

文字中出现 `[星星眼]`、`[呲牙]`、`[喜极而泣]`、`[打call]` 任一表情时，直接按本地规则屏蔽，不调用 Jev；同样识别表情图片的文字说明。主评论被屏蔽时，其楼中回复和“共 x 条回复，点击查看”入口一起隐藏。
...
- **表情直接屏蔽**优先于模型和缓存，对评论、弹幕均生效，不受隐藏阈值影响。隐藏记录会标明“表情规则”，不会伪造 Jev 分数。
- **Debug** 默认关闭。开启后即时在可见评论及回复的正文末尾用灰色字体显示 `Jev: 0.01` 这样的实际屏蔽概率，越高越倾向隐藏，不再依赖昵称结构。缓存命中时显示缓存值；切换开关不额外请求模型，分数标签也不会作为正文或父评论发送给 Jev。临时显示全部评论时，没有模型分数的内容会注明未调用、待判断或判断失败。
...
先判断主评论，只有主评论通过（包括命中有效缓存），才提交其楼中回复给 Jev。主评论未完成判断、被屏蔽、判断失败或无法识别时，整组回复与展开入口一起隐藏，后续加载的回复也不请求模型。主评论放行后，各条回复仍需分别通过筛选。

发送请求及重试前会再次检查主评论：若主评论在排队期间变化而不再通过，取消尚未发送的回复任务；以后主评论重新通过时恢复判断。已经发送的请求无法撤回，返回结果不会让被屏蔽的整组回复重新显示。暂停过滤或临时显示全部评论会解除显示遮挡，但临时显示不会绕过主评论通过后才提交回复的规则。
...
本版已通过 23 项纯逻辑/Chromium 夹具测试，覆盖主评论与回复的请求顺序、主评论失败时跳过回复、排队取消与恢复、四种表情绕过模型、整组回复与展开按钮隐藏、正文末尾灰色调试分数及输入隔离、原生样式与开关保留、节点复用、过期异步结果、分段数量、预取去重、接口失败、Canvas 遮挡和页面切换。旧版已完成一次真实 Jev 批量接口测试；本次未改变模型请求格式，也未重复付费接口测试。开发环境访问 B 站接口返回 HTTP 412，尚未完成登录状态下真实 B 站视频页的端到端验证；安装后请先查看面板中的弹幕状态。
